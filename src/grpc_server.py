...
class EnhancedEmbeddingService(embed_pb2_grpc.EmbeddingServiceServicer):
    def __init__(self, config: ServerConfig):
        ...
        self.ready_event = asyncio.Event()  # Signals readiness
        self.model_lock = asyncio.Lock()    # Prevents concurrent loads
    ...
    async def initialize(self):
        ...
        async with self.model_lock:
            with operation_context("service_initialization") as op:
                ...
                # Load default model (using unlocked version since we already hold the lock)
                if self.config.default_model:
                    with operation_context("default_model_loading", model=self.config.default_model) as model_op:
                        success = await self._load_model_unlocked(
                ...
                self.ready_event.set()  # Mark ready at end of initialization
    ...
    async def _load_model_unlocked(
        ...
        """Internal model loading without lock acquisition (for use within locked contexts)."""
    ...
    async def LoadModel(
        ...
        await self.ready_event.wait()  # Wait until server is ready
        ...
        with RequestContext(request_id, method="ProcessFileStream") as ctx:
            ...
            try:
                ...
                if not file_metadata:
                    ...
                    return FileStreamResponse(success=False, message="No file metadata received")
                ...
                if not file_content.strip():
                    ...
                    return FileStreamResponse(success=False, message="No text content found in file")
                target_model = model_name or self.model_name
                if not target_model:
                    logger.error("No model specified and no default model loaded")
                    return FileStreamResponse(success=False, message="No model specified and no default model loaded")
                ...
                if target_model != self.model_name:
                    with operation_context("model_switching", from_model=self.model_name, to_model=target_model):
                        ...
                        if not success:
                            ...
                            return FileStreamResponse(success=False, message=f"Failed to load model {target_model}")
                ...
                if not chunks:
                    logger.error("No valid chunks created from file content", 
                               filename=file_metadata.filename)
                    return FileStreamResponse(success=False, message="No valid chunks created from file content")
                with operation_context("embedding_generation", 
                                     chunks_count=len(chunks),
                ...
                logger.error("Error processing file stream", 
                           ...
                           duration=total_duration,
                           exc_info=True)
                return FileStreamResponse(success=False, message=f"Error processing file: {str(e)}")
    ...
    async def GetStatus(
        ...
        # Status checks should not block on readiness (reports current state)
        request_id = context.peer() + "-" + str(int(time.time() * 1000))
        with RequestContext(request_id, method="GetStatus") as ctx:
            ...
            # Create StatusResponse and populate the configuration map
            ...
            # Populate the protobuf map field correctly
            for key, value in config_info.items():
                response.configuration[key] = str(value)
    ...
    async def ListModels(
        ...
        # ListModels does not require readiness (only static info)
        request_id = context.peer() + "-" + str(int(time.time() * 1000))
